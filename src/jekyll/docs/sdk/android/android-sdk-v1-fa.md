...
<br/>
۱. وابستگی مربوط به کتابخانه متریکس را در قسمت `dependencies` فایل `gradle` اپلیکیشن خود اضافه کنید:

```groovy
 implementation 'ir.metrix:metrix:1.0.0-beta1'
```

۲. اضافه کردن `app id` متریکس در فایل `AndroidManifest.xml` اپلیکیشن:
...
<manifest ...>
  ...
  <application ...>
    ...
    <!-- خطوط زیر را اضافه کنید و APP_ID را جایگزین نمایید. -->
    <meta-data
...
```java
Metrix.newRevenue("my_event_slug", 12000, RevenueCurrency.IRR, "orderId");
```
...
**توجه:** این فراخوانی باید در متد `onCreate` در کلاس `Application` برنامه صورت گیرد. 
در صورتی که کلاس `Application` 
